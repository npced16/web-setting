      ...
      for (const data of content.col[item]) {
        ...
        if (data.type == 'room') {
          if (!currData[data.name]) {
            currData[data.name] = {
              startPoint: numberToAlphabet(item) + count,
              endPoint: ''
            }
          }
        }
        if (data.type == 'bed') currData[data.name].endPoint = numberToAlphabet(item) + count
      }
    }
  }
...
function settingContnets() {
  for (const list of itemList.data) {
...
const itemList = {
  "responseTime": "2024-01-22 14:19:21",
  "message": "roomList",
  "data": [{
